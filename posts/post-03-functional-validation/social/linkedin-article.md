...
If AI is writing your code AND your tests, your green CI is theater. Here's why I stopped accepting unit tests as evidence of completion, what replaced them, and the 642 times my enforcement system caught agents trying to write mirrors of their own assumptions.
...
> **Diagram: Three-Hook Enforcement Chain**
> - Agent writes code → tries to create a test/mock/stub? Hook 1 (`block-test-files.js`) blocks via 14 regex patterns. Fired 642 times across 23,479 sessions.
> - Agent runs a build command? Hook 2 (`validation-not-compilation.js`) injects "Compilation is NOT validation. Exercise the feature through real UI."
> - Agent claims task complete? Hook 3 (`completion-claim-validator.js`) searches for evidence. No screenshot or log → "BUILD SUCCESS ≠ VALIDATION. Screenshot or log required."
> - Only when evidence is found does the completion claim get accepted.
...
> **Diagram: Three-Layer Validation Stack**
> - Layer 1 (Compilation): TypeScript strict mode / ESLint / Swift compiler → "Compiles?" → PASS = structure valid
> - Layer 2 (Runtime): Start server → TCP connect → HTTP 200 → content marker check → PASS = correct app running
> - Layer 3 (Functional): Navigate → snapshot accessibility tree → click/tap/type → verify outcome → capture screenshot → PASS = VALIDATED
> - Each layer feeds the next; failure at any layer routes back to fix-and-retry before advancing.
...
*Originally published at: https://site-rho-pied.vercel.app/posts/post-03-functional-validation*
