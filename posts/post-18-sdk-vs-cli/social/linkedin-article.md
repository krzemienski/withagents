...
Every engineering leader I talk to is asking the same question right now: do we build on the Anthropic SDK, the Claude Code CLI, or the Agent SDK? Here's the framework I wish I'd had before I built the same pipeline three times.
...
> **Diagram: The Full Decision Tree** — New task → does it need file operations and codebase context? If yes: interactive or automated? Interactive → Claude Code CLI (full dev environment, MCP servers, hooks, skills). Automated → multi-agent coordination? Yes → Agent SDK (programmatic agents with built-in tools); no → Claude Code CLI. If no: batch processing or pipeline? Yes → need custom tool schemas? Yes → Anthropic SDK (direct API, precise control); no → Agent SDK. If not a pipeline: simple extraction or classification? Yes → Anthropic SDK. No → deterministic output format? Yes → Anthropic SDK; no → Hybrid (SDK for data ops, CLI for code ops).
...
> **Diagram: The Hybrid Architecture** — Three layers. The Application Layer (59 DB tables in Drizzle ORM, auth & RLS across 7 platforms, analytics, scheduling, a 6-dimension weighted scoring algorithm). The SDK Layer (content classification, metadata extraction, batch analysis of thousands of items — cost is in fractions of a cent per call). The CLI Layer (codebase refactoring, feature implementation, bug investigation — multi-file edits with MCP-powered tools). The app layer sends structured data to the SDK layer and receives enriched content back; the CLI layer pushes code changes into the app layer.
...
*Originally published at: https://site-rho-pied.vercel.app/posts/post-18-sdk-vs-cli*
