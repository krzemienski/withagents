...
If you manage AI-augmented engineers or set policy around AI coding tools, the gap between what you think your developers are doing and what the logs prove they're doing is probably wider than you expect. Here's what happened when I mined 11.6GB of my own Claude Code session data — and what it did to the numbers I'd been publishing.
...
> **Diagram: Mining Pipeline Overview**
> - Raw JSONL Files (23,479 files / 11.6GB) → Parser (line-by-line JSON decode) → Entry Extraction (tool calls, content, timestamps)
> - Entry extraction feeds both Tool Signature Vectorization and Content Signal Detection
> - Tool signatures → K-means topic clustering (k=20)
> - Both paths feed the 7-Dimension Scoring engine → Deduplication (title similarity filter) → Ranked Insights (composite score ≥ 0.3) → Mining Report
...
> - Stage 1 (Ingest): `discover_session_files()` → `parse_session_file()` → SessionLog objects
> - Stage 2 (Process): `cluster_entries()` → `score_entries()` → `categorize()`
> - Stage 3 (Output): `_deduplicate()` → MiningReport → ranked insights
...
*Originally published at: https://site-rho-pied.vercel.app/posts/post-09-code-tales*

*One of 18 essays in "Agentic Development: 18 Lessons from 23,479 AI Coding Sessions." Companion code: github.com/krzemienski/session-insight-miner*
