...
subtitle: "How structured hypothesis-test-revise chains solve bugs that brute force debugging never will"
...
```mermaid
flowchart TD
    subgraph phase1["Phase 1: Constraint Establishment (Steps 1-22)"]
        S1["Step 1: Observe 12.5% = 1/8 failure rate"]
        S5["Step 5: Map 4-layer architecture"]
        S12["Step 12: Enumerate all sources of '8' in system"]
        S1 --> S5 --> S12
    end

    subgraph phase2["Phase 2: Category Elimination (Steps 23-46)"]
        S23["Step 23: ELIMINATE race conditions<br/><i>Race conditions produce variable rates, not fixed 1/8</i>"]
        S31["Step 31: ELIMINATE encoding bugs<br/><i>Encoding errors affect all plays, not 12.5%</i>"]
        S39["Step 39: ELIMINATE player bugs<br/><i>Player sees correct data — corruption is upstream</i>"]
        S23 --> S31 --> S39
    end

    subgraph phase3["Phase 3: Layer-by-Layer Trace (Steps 47-67)"]
        S47["Step 47: CDN anomaly found<br/><i>Unusual byte-range caching policy</i>"]
        S48["Step 48: REVISION — unusual ≠ root cause<br/><i>CDN response is correct given its input</i>"]
        S55["Step 55: Trace backward from CDN to API gateway"]
        S62["Step 62: Trace backward from gateway to PostgreSQL"]
        S47 --> S48 --> S55 --> S62
    end

    subgraph phase4["Phase 4: Root Cause Confirmation (Steps 68-84)"]
        S68["Step 68: BREAKTHROUGH<br/><code>offset = file_size // 8</code>"]
        S72["Step 72: WAV header = 44 bytes<br/><i>Chunk 1 includes header as audio data</i>"]
        S78["Step 78: Verify: 1/8 chunks hit chunk 1 = 12.5%"]
        S84["Step 84: ROOT CAUSE CONFIRMED<br/><i>Fix: offset = (file_size - 44) // 8 + 44</i>"]
        S68 --> S72 --> S78 --> S84
    end

    phase1 --> phase2 --> phase3 --> phase4

    style S23 fill:#ef4444,color:#fff
    style S31 fill:#ef4444,color:#fff
    style S39 fill:#ef4444,color:#fff
    style S48 fill:#f59e0b,color:#000
    style S68 fill:#22c55e,color:#fff
    style S84 fill:#22c55e,color:#fff
```
...
```mermaid
flowchart LR
    subgraph cycle["Hypothesis-Test-Revise Cycle"]
        direction TB
        H["HYPOTHESIZE<br/><i>State what you think<br/>is happening and why</i>"]
        T["TEST<br/><i>Make a specific prediction<br/>that would be true if<br/>the hypothesis holds</i>"]
        D{Prediction<br/>matches<br/>reality?}
        E["ELIMINATE<br/><i>State which constraint<br/>the hypothesis violates</i>"]
        R["REVISE<br/><i>Update understanding<br/>based on what was learned</i>"]
        C["CONFIRM<br/><i>Root cause explains<br/>all constraints</i>"]
    ...
    subgraph constraints["Constraint Filter"]
        Q1["Does it explain the<br/>exact frequency?"]
        Q2["Does it explain which<br/>layer is affected?"]
        Q3["Does it explain why<br/>other layers appear normal?"]
        Q1 --- Q2 --- Q3
    end

    constraints -.->|"Applied at<br/>every step"| D
...
*The [sequential-thinking-debugging](https://github.com/krzemienski/sequential-thinking-debugging) companion repo has the full framework: `DebuggingChain` with typed steps, hypothesis tracking, constraint propagation, and report generation. Clone it, feed it a bug with a quantitative constraint, and watch it eliminate hypothesis categories faster than you can type them. The WAV bug demo runs in a single command: `seq-debug demo`.*
