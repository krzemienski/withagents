...
1. **Which files can each agent touch?** Agents working in parallel reach for shared infrastructure files — `package.json`, type indices, route registries — because that's where integration happens. Without explicit ownership, everyone touches everything.
...
```mermaid
flowchart TD
    START([Task Assigned to Agent]) --> CHECK{File in agent's\nownership zone?}
    CHECK -->|Yes| WRITE[Agent modifies file\nin its worktree]
    CHECK -->|No| LOOKUP[Lookup file owner]
    LOOKUP --> OWNED{File has\nan owner?}
    OWNED -->|Yes| REJECT[OwnershipViolation\nAgent blocked]
    OWNED -->|No| UNOWNED[Flag as unowned\nAssign to integrator]
    REJECT --> REASSIGN[Restructure task or\nreassign file ownership]
    REASSIGN --> CHECK
    UNOWNED --> INTEGRATOR[Integrator agent\ntakes ownership]
    INTEGRATOR --> CHECK
    WRITE --> DIFF[Git diff captures\nchanged files]
    DIFF --> PREDICT{Any overlap with\nother worktrees?}
    PREDICT -->|No| SAFE[Safe to merge]
    PREDICT -->|Yes| CONFLICT[Conflict predicted\nbefore merge attempt]
    CONFLICT --> REASSIGN
    SAFE --> MERGE[Enter merge\nchoreography]

    style START fill:#6366f1,color:#f1f5f9
    style SAFE fill:#22c55e,color:#0f172a
    style REJECT fill:#ef4444,color:#f1f5f9
    style CONFLICT fill:#ef4444,color:#f1f5f9
    style MERGE fill:#22d3ee,color:#0f172a
```
...
graph TD
    ...
    style DB fill:#6366f1,color:#f1f5f9
    style CFG fill:#6366f1,color:#f1f5f9
    style AUTH fill:#8b5cf6,color:#f1f5f9
    style MEDIA fill:#8b5cf6,color:#f1f5f9
    style STORAGE fill:#8b5cf6,color:#f1f5f9
    style IOS fill:#22d3ee,color:#0f172a
    style WEB fill:#22d3ee,color:#0f172a
    style ADMIN fill:#22d3ee,color:#0f172a
    style INT fill:#22c55e,color:#0f172a
```
...
*The [multi-agent-merge-orchestrator](https://github.com/krzemienski/multi-agent-merge-orchestrator) repo contains the full system: `OwnershipMatrix` for file zone enforcement, `predict_conflicts` for pre-merge conflict detection, `topological_sort` for dependency-ordered merging, and `execute_merge_choreography` for the automated merge-verify pipeline. It includes a CLI (`merge-orchestrator`) and a basic usage example with four agents and intentional ownership violations to demonstrate the validator.*
