...
If you've built internal tools, you know the arc: scrappy script, bloated app, specs, eventual rewrite. I watched my AI builder go through that same four-generation loop — and the fourth generation wasn't a builder at all.
...
> **Diagram: CCB Evolution Timeline** —
> - **Gen 1 — Bash (May 2025):** 47-line prototype grew to 955 lines, 12 sequential phases, $47 infinite loop incident, "NO MOCKS" instruction file born.
> - **Gen 2 — Electron (Aug 2025):** Desktop GUI application, 19 Zustand stores, 329 IPC handlers, 596 channel definitions, 4 parallel agents just to map it.
> - **Gen 3 — Mobile Specs (Mar 2026):** Specification-as-builder, 2 competing specs in parallel, 8,695 lines across 4 docs, 380 REST + 120 WebSocket endpoints.
> - **Gen 4 — CLI Native (Mar 2026):** `claude --print` replaces wrappers, skills + hooks + MCP ecosystem, SDK for pipelines, CLI for reasoning, the builder becomes the tool.
...
> **Diagram: Four Generations Side-by-Side** —
> - **Gen 1 (Bash Script, 955 lines):** Prompt → `claude --print` → build check → next phase on pass, retry (≤3x) on fail.
> - **Gen 2 (Electron App, 19 stores, 329 IPC):** React UI connected via 596 IPC channels to Main Process, which talks to Python Backend, 19 Zustand Stores, and XState Machines.
> - **Gen 3 (Mobile Specs, 8,695 lines):** Existing app feeds two agents in parallel (Agent A: React Native produces a 2,870-line spec; Agent B: SwiftUI produces a 5,117-line spec).
> - **Gen 4 (CLI + SDK Native):** SDK (orchestration) drives `claude -p` (execution) plus skills/hooks/MCP and parallel worktrees, producing results.
> Gen 1 → Gen 2 because UI was needed. Gen 2 → Gen 3 because mobile was needed. Gen 3 → Gen 4 because the tool absorbed the patterns.
...
*Originally published at: https://site-rho-pied.vercel.app/posts/post-17-ccb-evolution*

*One of 18 essays in "Agentic Development: 18 Lessons from 23,479 AI Coding Sessions." Companion code: github.com/krzemienski/claude-code-monorepo*
