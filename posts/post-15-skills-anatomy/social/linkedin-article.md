...
Every team I talk to has a "prompt library" that nobody uses. Here's what changed when I stopped treating prompts as text and started treating them as programs — with inputs, routing, and exit gates.
...
> **Diagram: The Three-Layer Skill Model** —
> - **Layer 1 (always in context, ~100 tokens):** name + description, the trigger surface.
> - **Layer 2 (loaded on trigger, ~200-500 lines):** Essential Principles (3-5 override rules), Intake Menu (user interaction gate), Routing Table (condition → agent mapping), Execution Steps (numbered workflow), Success Criteria (completion checklist).
> - **Layer 3 (loaded on demand, unlimited):** references/ (domain knowledge files), scripts/ (executable code), templates/ (output templates), workflows/ (multi-step protocols). Steps pull from Layer 3 as needed.
...
> **Diagram: Skill Resolution Sequence** — User says "validate this feature." Claude Code matches against all skill descriptions and returns ranked matches (functional-validation 0.94, gate-validation-discipline 0.71, e2e-validate 0.65). It loads functional-validation/SKILL.md, which returns principles, intake, routing, steps, and criteria. Claude presents the intake menu ("What platform? iOS / Web / CLI / API"). User picks "iOS." Claude routes to the executor + simulator stack. For each execution step, Claude executes and checks the step's validation gate — on pass, proceed; on fail, report and request guidance. Finally it checks all success criteria and produces a completion report with evidence.
...
*Originally published at: https://site-rho-pied.vercel.app/posts/post-15-skills-anatomy*

*One of 18 essays in "Agentic Development: 18 Lessons from 23,479 AI Coding Sessions." Companion code: github.com/krzemienski/claude-code-skills-factory*
