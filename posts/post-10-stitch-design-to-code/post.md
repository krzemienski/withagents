---
title: "The Designer-Less Design Workflow: Stitch MCP and the Death of Figma Handoffs"
subtitle: "269 AI-generated screens, zero Figma files, and the branding bug that taught me to treat prompts as build artifacts"
...
```mermaid
flowchart TD
    A["DESIGN.md\n(Design System Spec)"] -->|"Embed full token set\nin every prompt"| B["Craft Structured Prompt\nDevice type + tokens +\ncomponents + screen desc"]
    B -->|"generate_screen_from_text\n(projectId, prompt, deviceType)"| C["Stitch MCP\nGenerates rendered screen"]
    C -->|"get_screen(screenId)"| D["Review Output\nCheck brand, colors,\nlayout, typography"]
    D -->|"Matches spec?"| E{Pass?}
    E -->|No| F["Adjust Prompt\nFix specific deviations:\nradius, color, font"]
    F --> B
    E -->|Yes| G["Convert to React\nshadcn/ui + Tailwind +\ndesign token preset"]
    G --> H["Validate\nPuppeteer 107-action suite\nStructure + interaction + screenshot"]
    H --> I{All checks pass?}
    I -->|No| J["Fix Component\nor Regenerate"]
    J --> G
    I -->|Yes| K["Ship It"]
    ...
    style E fill:#0f172a,stroke:#f59e0b,color:#f1f5f9
    style F fill:#1e293b,stroke:#ef4444,color:#f1f5f9
    style G fill:#1e293b,stroke:#6366f1,color:#f1f5f9
    style H fill:#1e293b,stroke:#22d3ee,color:#f1f5f9
    style I fill:#0f172a,stroke:#f59e0b,color:#f1f5f9
    style J fill:#1e293b,stroke:#ef4444,color:#f1f5f9
    style K fill:#1e293b,stroke:#059669,color:#f1f5f9
```
...
```mermaid
flowchart LR
    A["tokens.json\n47 design tokens\nColors, type, spacing,\nshadows, radii"] -->|"require()"| B["tailwind-preset.js\nMaps tokens to\nTailwind utilities"]
    A -->|"Inline in prompt"| C["Stitch Prompts\nFull spec embedded\nin every generation call"]
    B -->|"tailwind.config.js\npresets: [preset]"| D["Tailwind CSS\nbg-background\ntext-primary\nfont-mono"]
    D --> E["React Components\nshadcn/ui primitives\nCVA variants\ndata-testid attrs"]
    C -->|"generate_screen_from_text"| F["Stitch Output\nRendered HTML\nVisual reference"]
    F -->|"Convert structure"| E
    E --> G["21 Production Screens\nConsistent tokens\nZero hardcoded values"]

    style A fill:#1e293b,stroke:#22d3ee,color:#f1f5f9
    style B fill:#1e293b,stroke:#6366f1,color:#f1f5f9
    style C fill:#1e293b,stroke:#6366f1,color:#f1f5f9
    style D fill:#1e293b,stroke:#6366f1,color:#f1f5f9
    style E fill:#1e293b,stroke:#6366f1,color:#f1f5f9
    style F fill:#1e293b,stroke:#22d3ee,color:#f1f5f9
    style G fill:#1e293b,stroke:#059669,color:#f1f5f9
```
