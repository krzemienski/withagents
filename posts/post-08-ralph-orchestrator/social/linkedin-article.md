...
If you're trying to run AI agents overnight without waking up to chaos, here's the pattern that let me finish 28 of 30 tasks between midnight and morning: one hat per session, one event per hat, filesystem as memory. The agent never decides when to stop — the task list does.
...
> **Diagram: Hat State Machine**
> - Task arrives → Planner hat (reads codebase, emits plan)
> - `plan.complete` event → Builder hat (loads plan, writes code, emits build)
> - `build.complete` event → Reviewer hat (loads code, critiques, emits verdict)
> - 0 critical issues → Converged (loop terminates)
> - Issues found → Fixer hat (loads issues, applies fix, emits fix)
> - `fix.complete` event → back to Reviewer
...
> **Diagram: Three Orchestration Modes**
> - Ralph (Single Agent Loop): Plan → Build → Review → Issues? → Fix → Review → Done
> - Ultrawork (Parallel Agents): Plan → Build Task A + Build Task B + Build Task C simultaneously → Merge & Review → Done
> - Team Mode (Coordinated N Agents): Lead plans & writes PRD → Dev 1 (Feature A), Dev 2 (Feature B), Tester (Verify) in parallel → Lead merges → Done
...
*Originally published at: https://site-rho-pied.vercel.app/posts/post-08-ralph-orchestrator*

*One of 18 essays in "Agentic Development: 18 Lessons from 23,479 AI Coding Sessions." Companion code: github.com/krzemienski/ralph-loop-patterns*
