...
If you build iOS and you've ever been burned by a streaming architecture that looked right on paper and died in production, this one's for you. Four failed attempts, ten network hops, and the counterintuitive lesson that more layers produced fewer failure modes.

I wanted one thing: Claude streaming into a SwiftUI view, token by token. Four failed architectures, a five-layer bridge with ten network hops per token, and 7,985 iOS MCP tool calls later, I had something that actually worked.
...
> **Diagram: Five-Layer Streaming Bridge**
> - iOS Device: SwiftUI App with `@Observable ChatViewModel`
> - Local Backend: Vapor Server (Swift/NIO HTTP) + ClaudeExecutorService (actor, GCD stdout reader)
> - Python Bridge: `sdk-wrapper.py` — 8 lines of glue code
> - Claude Infrastructure: Claude CLI with OAuth token chain + Anthropic API for model inference
> - Request path: SwiftUI → HTTP POST `/api/v1/chat/stream` → Vapor → Process.spawn() → Python → claude_agent_sdk.query() → CLI → authenticated HTTPS → API
> - Response path reverses: tokens → CLI stdout → Python stdout → NDJSON → StreamMessage decode → SSE event stream → SwiftUI view update
...
Every layer exists because I tried to remove it and the system broke. Remove Python and Swift can't authenticate. Remove Vapor and the app can't maintain a persistent SSE connection to a subprocess. Remove the `StreamMessage` type layer and snake_case JSON from Python crashes the Swift decoder. Remove the ViewModel and `@Observable` bindings fire on every SSE line, flooding SwiftUI with 200 re-renders per second.

Here's the counterintuitive part: more layers meant fewer failure modes. Each layer does exactly one translation. When a bug shows up, it lives in exactly one layer, and the layer boundaries tell you which one.
...
> **Diagram: SSE Connection State Machine**
> - States: Disconnected, Connecting, Connected, Reconnecting
> - Disconnected → Connecting on `startStream()`
> - Connecting → Connected on HTTP 200 + first byte; Connecting → Disconnected on 60s timeout; Connecting → Reconnecting on network error
> - Connected → Reconnecting on 45s heartbeat timeout or network error; Connected → Disconnected on stream complete or `cancel()`
> - Reconnecting → Connecting on backoff elapsed; Reconnecting → Disconnected after max 3 attempts or `cancel()`
> - Notes: 60s race between `URLSession.bytes()` and `Task.sleep`; `LastActivityTracker` uses `OSAllocatedUnfairLock` for thread-safe timestamp; exponential backoff `min(base * 2^attempt, 30s)` is cancellable
...
I almost scrapped the Python bridge entirely and rewrote it in Go before realizing the problem was a single missing function call. Think about that for a second. I was ready to throw away an entire architecture layer over a buffer flush.
...
5. **Use two-tier timeouts.** A stuck process (no output) and a long process (continuous output) need different timeout strategies. One timeout can't serve both purposes.

6. **Cancel on background.** On iOS, don't try to maintain SSE connections when the app is backgrounded. Cancel cleanly and reconnect on foreground. The alternative is battery drain and App Store rejection.
