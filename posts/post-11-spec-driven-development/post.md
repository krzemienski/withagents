---
title: "Spec-Driven Development: Why YAML Beats Verbal Instructions for AI Agents"
subtitle: "A single YAML file replaces meetings, tickets, and Slack threads — agents read it, build in parallel, and ship without asking clarifying questions"
author: "Nick Krzemienski"
date: "2026-03-06"
series_number: 11
series_total: 18
github_repo: "https://github.com/krzemienski/reponexus"
tags:
  - AgenticDevelopment
  - ClaudeCode
  - SpecDriven
...
```mermaid
flowchart TD
    A["YAML Spec File"] --> B["Parse & Validate<br/>(JSON Schema)"]
    B --> C["Dependency Graph<br/>(Topological Sort)"]
    C --> D["Layer 0: No Dependencies<br/>(Build in Parallel)"]
    C --> E["Layer 1: Depends on Layer 0<br/>(Build in Parallel)"]
    C --> F["Layer N: Depends on Layer N-1<br/>(Build in Parallel)"]
    D --> G["Gate Evaluation<br/>(type-check → contract → integration → functional)"]
    E --> G
    F --> G
    G -->|All Gates Pass| H["Ship"]
    G -->|Gates Fail| I["Retry Failed Components<br/>(max 2 retries)"]
    I --> G
    ...
    style D fill:#0f766e,stroke:#14b8a6,color:#f1f5f9
    style E fill:#0f766e,stroke:#14b8a6,color:#f1f5f9
    style F fill:#0f766e,stroke:#14b8a6,color:#f1f5f9
    style G fill:#b45309,stroke:#f59e0b,color:#f1f5f9
    style H fill:#15803d,stroke:#22c55e,color:#f1f5f9
    style I fill:#b91c1c,stroke:#ef4444,color:#f1f5f9
```
...
stateDiagram-v2
    [*] --> Discover
    Discover --> Plan: components discovered
    Plan --> Execute: spec validated
    Execute --> Verify: all agents complete
    Verify --> Ship: all gates pass
    Verify --> Execute: gates fail (max 2 retries)
    Ship --> [*]

    state Discover {
        [*] --> MapCodebase
        MapCodebase --> IdentifyComponents
        IdentifyComponents --> [*]
    }

    state Plan {
        [*] --> ParseSpec
        ParseSpec --> ResolveDependencies
        ResolveDependencies --> ValidateContracts
        ValidateContracts --> [*]
    }

    state Execute {
        [*] --> SpawnAgents
        SpawnAgents --> ParallelBuild
        ParallelBuild --> CollectResults
        CollectResults --> [*]
    }

    state Verify {
        [*] --> RunTypeChecks
        RunTypeChecks --> RunContractGates
        RunContractGates --> RunIntegrationGates
        RunIntegrationGates --> RunFunctionalGates
        RunFunctionalGates --> [*]
    }

    state Ship {
        [*] --> MergeWorktrees
        MergeWorktrees --> FinalValidation
        FinalValidation --> [*]
    }
```
