#!/usr/bin/env python3
"""Mine last-30-days Claude Code sessions. Thin wrapper over deep-mine.py logic
filtered by mtime, writes to scripts/output/mine-30d-data.json."""
...
def main():
    ...
    print(f"[30d] Found {len(all_files)} JSONL files modified in last 31 days", file=sys.stderr)
    ...
    for idx, (fp, size, mtime) in enumerate(all_files, 1):
        ...
        if idx % 150 == 0:
            print(f"[30d] {idx}/{len(all_files)}", file=sys.stderr)
    ...
    for pname, pd in project_data.items():
        ...
        out["projects"][pname] = {
            ...
            "error_samples": list({e[:150] for e in pd["errors"]})[:10],
        }
    ...
    with open(outdir / "mine-30d-data.json", "w") as f:
        json.dump(out, f, indent=2)
    print(f"[30d] wrote {outdir / 'mine-30d-data.json'}", file=sys.stderr)
